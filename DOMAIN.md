...
#### Bounded-buffer assembly line (Runtime redesign ④ — foundation landed in ④a, migration 012)
- **Run** — the `Run` aggregate (root `run_id`): one execution of a pipeline — the tree of work-items produced from a single Start. Owns the *completes-exactly-once* invariant, enforced by the conditional-UPDATE guard `UPDATE runs SET completed=1 WHERE id=? AND completed=0` (the same single-writer idiom as the `FanOutGroup` barrier). Tracks a `generator_dry` flag; the completion **precondition** (generator dry AND all stores empty AND no running workers) is computed by the caller (④b/④h) — `try_complete` is only the guard. (`runtime::run_store`.)
