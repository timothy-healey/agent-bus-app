...
### LF28 · Usage meter overall stuck at 0% — cc_usage_log ingestion never wired — `uninvestigated→root-caused`

> Hovering the usage bar shows a by-team breakdown with real tokens, but the headline reads 0% / 0 tok / 0/min. Root cause: the **overall window total + burn** read only `cc_usage_log` (`snapshot.rs:84` — *"v1 window total = cc_usage_log; + API worker rows (zero in v1)"*), while the **by-team** breakdown reads the separate `worker_usage_log`. `cc_usage_log` is fed by ingesting Claude Code transcripts (`parse_transcript` + `CcUsageStore::ingest`), but **`.ingest()` is called only in tests** — there is no watcher/poller, no `read_dir`, no `~/.claude/projects` reference in production. So `cc_usage_log` is permanently empty (live DB: 0 rows) and the headline meter has always shown 0%, even though `worker_usage_log` has real per-invocation counts (live: research 9947+28592, spec-writers 9099+7413). The transcripts exist on disk (`~/.claude/projects/<cwd-hash>/<session>.jsonl`) — the `--print` worker runs do write them — so D3 ingestion is viable. NOTE: the worker transcripts currently land under the app-cwd hash dir (LF26); fixing cwd moves them, so the ingester must scan ALL of `~/.claude/projects/` (or be cwd-aware), not hardcode one project dir. Open design decision: should the window total count ALL Claude Code activity (true shared-subscription window the brake protects) or only the app's own worker runs?

### LF29 · Run control should show "Running" + Stop, not just "Start run" — `open`
> RunSelector shows "Start run" (disabled→"starting…"); Stop is a separate topbar brake toggle. Want: once a run is active, the control reads "● Running" with an inline Stop so the operator can halt at any point. Stop today = brake (blocks new claims); the lifecycle spec upgrades it to also kill in-flight work. The button-state UI is independent of that and can ship now.

### LF30 · CardDrawer close ✕ overlaps the header's `stage · aN` text — `fixed`
> The `Drawer` close ✕ is absolute at `top:10 / right:12`; the CardDrawer header's top row puts `{current_stage} · a{attempts}` flush right (justify space-between), so the ✕ overlaps it. Same class as LF18. Fix: right-pad the header top row to clear the ✕.

### LF31 · No card for the generator/research source — `deferred → live-worker-view spec`
> The source/generator pass has no board card (nothing to click while it scans). This is exactly the "generator as a clickable transient card" in `docs/superpowers/specs/2026-06-25-live-worker-view-design.md` (§C) — ships with that spec.

### LF32 · Card leads with the opaque `task.id`; body (topic) is empty; slug not shown — `open`
> The orange string at the top of a card is the raw `task.id` (`T-<uuid>`) — meaningless to the operator. Worse, `topic` is empty for generated work-items (live DB: all `[]`), so the card body is blank; the only human-meaningful identifier, `item_key` (the slug, e.g. `LWV-A1-logdelta-seam`), is shown nowhere. Redesign: lead the card with a short plain-language description, show the slug as secondary metadata, drop the raw `task.id` from the prominent slot. Needs a description source — the generator already writes a titled artifact (`# Candidate … — Title`); cleanest is to have the output contract emit a one-line `DESCRIPTION:` per item, stored on the work-item and rendered as the card title. Pairs with the live-worker-view card work.

### LF33 · Frontend "Start run" doesn't emit run-changed → board doesn't auto-refresh + run proliferation — `open→building`
> The frontend Start button calls `runtime::api::start_run` registered RAW in the invoke_handler (`lib.rs:1849`); the runtime crate can't emit Tauri events and it was never wrapped at the root, so creating a run emits NO `run-changed`. `useRuns` only refetches on `run-changed`, so `hasRun` stays false → BoardView shows "no run scoped" and the run-control stays "Start run" (never flips to ● Running) even while the worker loop runs + emits `task-changed`. The operator, seeing nothing, presses Start again → a SECOND run. A manual reload re-fetches `listRuns` and shows everything. (The `/inject` dispatcher path DOES emit run-changed at `lib.rs:1171`; only the frontend button path is broken.) This is the live root of the earlier LF23 "nothing happens". Fix (run-model chunk): root `start_run` wrapper that is idempotent (one active run; resume a braked run, never spawn a parallel) + emits run-changed; demote the run dropdown to history; keep Run internal.

### LF34 · Usage meter undercounts — ignores cache tokens — `open→building`
> `window_tokens` = `SUM(input_tokens + output_tokens)` only (`cc_log.rs:60`); cache is ingested but never counted. Live 5h window: input+output 347,519 vs cache_creation 763,225 + cache_read **66,069,475** — so the meter reads 347k/2.6M budget ≈ 13% while claude.ai shows ~35%. The brake is blind to ~99% of the tokens flowing (cache-heavy agent runs) → under-protects. Calibration from the data: total throughput (incl. cache) = 67.2M; 67.2M/0.35 ≈ 192M, so counting ALL tokens against a ~190M budget reads ~35% and tracks claude.ai. Fix (usage chunk): `window_tokens`/`recent_tokens` count input+output+cache_creation+cache_read; recalibrate the default `window_budget` (~190M); expose window_budget in Settings to calibrate. A local estimate, never an exact mirror (claude.ai's formula + real plan limit are not queryable — G6).
