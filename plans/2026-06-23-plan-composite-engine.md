...
> - **VF1 (vet F1, `adds-where-a-refactor-fits`):** `extract_tool_call_block` (DD3/Task 1) re-implements `pipeline::design_session::extract_json_block`. Fork: (a) lift a shared extractor both `pipeline` + `app` call, or (b) accept the duplication with an explicit "keep in sync" cross-reference comment. **RESOLVED — operator chose (b):** `extract_tool_call_block` is kept LOCAL to `app/src/lib.rs` with a one-line doc-comment cross-referencing `pipeline::design_session::extract_json_block` so the intentional duplication is documented. The agentic loop stays decoupled from the wizard module.
> - **VF2 (vet F2, `off-language-naming`):** `ParsedToolCall` (Task 2) shadows the kernel's `ToolCallRequest`. Fork: rename (e.g. `ToolCallEmit`) or keep. **RESOLVED — operator chose to DROP `ParsedToolCall` entirely:** the model's fenced `{ "tool", "args" }` block is parsed directly into the canonical `agent_bus_core::ToolCallRequest { tool_name, args }` via a local `parse_tool_call` helper (no shadow type). `CompositeEngine`/`AgenticChatEngine` kept (acknowledged sound root-strategy names matching the existing `*Engine` suffix).
